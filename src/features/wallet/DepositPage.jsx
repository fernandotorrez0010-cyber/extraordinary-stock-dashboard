...
export default function DepositPage() {
  ...
  useEffect(() => {
    const unsub = subscribeCryptoMethods((data) => {
      const active = data.filter((m) => m.status !== "inactive");
      setMethods(active);
      if (active.length && !method) setMethod(active[0]);
    });
    return unsub;
  }, []); // eslint-disable-line
  ...
  return (
    <div>
      <PageHeader
        title="Deposit Funds"
        subtitle="Send funds and submit proof of payment"
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card p-6">
          ...
          <div className="space-y-2 mb-6">
            {methods.map((m) => (
              ...
              >
                <div
                  className={`w-3 h-3 rounded-full border-2 ${method.id === m.id ? "border-green-400 bg-green-400" : "border-slate-600"}`}
                />
        ...
        {methods.length === 0 && (
          <div className="card p-8 text-center text-slate-500 text-sm">
            No deposit methods configured yet. Contact support.
          </div>
        )}
        {/* Submit form */}
        <div className="card p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="p-3 rounded-xl bg-yellow-500/10 border border-yellow-500/20 text-xs text-yellow-300">
              ⚠️ Your deposit will be reviewed and credited within 24 hours
              after confirmation.
            </div>
