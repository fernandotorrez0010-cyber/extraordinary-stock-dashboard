...
export default function AdminTransactionsPage() {
  ...
  return (
    <div>
      <PageHeader title="All Transactions" subtitle={`${transactions.length} total`} />

      {/* Totals */}
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Total Deposited', value: totals.deposit, color: 'text-blue-400' },
          { label: 'Total Withdrawn', value: totals.withdraw, color: 'text-orange-400' },
          { label: 'Total Profit Assigned', value: totals.profit, color: 'text-green-400' },
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800">
                  {['User ID', 'Type', 'Amount', 'Method', 'Status', 'Reference', 'Date'].map(h => (
              ...
              <tbody>
                {filtered.map(tx => (
                  <tr key={tx.id} className="border-b border-slate-800/60 hover:bg-white/[0.02]">
                    ...
                    <td className="px-4 py-3.5"><Badge status={tx.type} /></td>
                    <td className="px-4 py-3.5 font-bold font-mono text-slate-200">{formatCurrency(tx.amount)}</td>
                    <td className="px-4 py-3.5 text-slate-400 text-xs">{tx.method || '—'}</td>
                    <td className="px-4 py-3.5"><Badge status={tx.status} /></td>
                    <td className="px-4 py-3.5 text-slate-500 text-xs font-mono">{tx.reference}</td>
